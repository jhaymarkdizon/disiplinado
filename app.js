...
if (window.supabase && SUPABASE_URL && !SUPABASE_URL.includes('YOUR_')) {
  try {
    ...
    console.warn('Supabase initialization failed, running offline local storage fallback:', e);
  }
}
...
async function loadUserStateFromSupabase(userId) {
  ...
  try {
    ...
    console.warn('Failed to load user state from Supabase:', e);
    return null;
  }
}

async function saveUserStateToSupabase(userId, state) {
  ...
  try {
    ...
    if (error) console.warn('Supabase state upsert error:', error);
  } catch (e) {
    console.warn('Failed to sync state to Supabase:', e);
  }
}

async function logTransactionToSupabase(userId, tx) {
  if (!supabaseClient) return;
  try {
    await supabaseClient.from('transactions').insert({
      ...
      description: tx.label || tx.description || '',
      type: tx.type || 'expense'
    });
  } catch (e) {
    console.warn('Failed to log transaction to Supabase table:', e);
  }
}
...
function saveUserStateLocal(userId, state) {
  try {
    localStorage.setItem(`disciplined_vault_user_${userId}`, JSON.stringify(state));
  } catch (e) {
...
async function loadUserState(userId) {
  let state = await loadUserStateFromSupabase(userId);
  if (!state) {
    state = loadUserStateLocal(userId);
  }
  return state;
}
...
async function saveState() {
  ...
  saveUserStateLocal(currentUser.id, appState);
  await saveUserStateToSupabase(currentUser.id, appState);
}
...
function setupEntryModal() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    await saveState();
    await logTransactionToSupabase(currentUser.id, { amount, category: txCategory, label, type });
  ...
  signinForm.addEventListener('submit', async (e) => {
    ...
    if (supabaseClient && supabaseClient.auth) {
      try {
        ...
        if (error) {
          showAuthAlert(error.message || 'Invalid email or password.');
          return;
        }
        ...
        showAuthAlert('Authentication service error. Please check connection.');
      }
    } else {
      showAuthAlert('Supabase connection required for authentication.');
    }
  });
    ...
    if (supabaseClient && supabaseClient.auth) {
      try {
        ...
        if (data?.user) {
          ...
          showAuthAlert('Registration successful! Please check your email or sign in.', 'success');
          setTimeout(() => {
            signupForm.classList.add('hidden');
            signinForm.classList.remove('hidden');
            authTitle.textContent = 'Sign In to Disciplined';
        ...
        showAuthAlert('Registration service error.');
      }
    } else {
      showAuthAlert('Supabase connection required for account creation.');
    }
  });
  ...
  forgotForm.addEventListener('submit', async (e) => {
    ...
    if (supabaseClient && supabaseClient.auth) {
      try {
        ...
        if (error) {
          showAuthAlert(error.message || 'Password reset request failed.');
          return;
        }
        showAuthAlert('Password reset email sent! Follow instructions in your inbox.', 'success');
      } catch (err) {
        showAuthAlert('Password reset service error.');
      }
    } else {
      showAuthAlert('Supabase connection required for password reset.');
    }
  });
...
async function checkAuthSession() {
  ...
  if (supabaseClient && supabaseClient.auth) {
    try {
      ...
      if (session?.user) {
        let profileName = session.user.user_metadata?.full_name || session.user.email.split('@')[0];
        const { data: profData } = await supabaseClient.from('profiles').select('name').eq('id', session.user.id).single();
        if (profData && profData.name) profileName = profData.name;
